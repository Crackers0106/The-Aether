...
public class CrystalTreeIslandFeature extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
                    ...
                    if ((float)(x * x + z * z) <= (f + 1.0F) * (f + 1.0F)) {
                        if (y == 0) {
                            ...
                            if (context.getRandom().nextInt(6) == 0) {
                                this.setBlockState(context.getWorld(), context.getOrigin().add(x, y+1, z), AetherBlocks.WHITE_FLOWER.getDefaultState());
                            }
                        } else if (y == -1) {
        ...
        for (int y = 1; y < 9; y++) {
            ...
            this.setBlockState(context.getWorld(), context.getOrigin().up(y), AetherBlocks.SKYROOT_LOG.getDefaultState());
        }
